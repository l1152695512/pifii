...
*/
    ...
    afterRender : function(){
        ...
        if (typeof this.addGOverViewMap === 'object' && this.gmapType === 'map') {
            this.gmap.addControl(this.addGOverViewMap);
         }
    },
    // private
    ...
    onMapReady : function(){
        ...
        google.maps.event.addListener(this.getMap(), 'click',function(e){
        
       	var maps=Ext.getCmp('panelMap');
       	if(Ext.isEmpty(maps)){
         ...
         var events=e.latLng;
       	...
       	var xCoord=Ext.getCmp('XCoord');
       	var yCoord=Ext.getCmp('YCoord');
       	var address=Ext.getCmp('addressMap');
       	xCoord.setValue(x);
       	yCoord.setValue(y);
       ...
       geocoder.geocode({latLng: latlng}, function(results, status) {
           if (status == google.maps.GeocoderStatus.OK) {
               if (results[3]) {
                  ...
                  var addressGeoCode=results[0].formatted_address;
                  address.setValue(addressGeoCode);
               }
           }
       });
            ...
            for (var i = 0; i < markers.length; i++) {
                if (markers[i]) {
                    if (typeof markers[i].geoCodeAddr == 'string') {
                        ...
                        this.addMarker(mkr_point, markers[i].marker, false,markers[i].icon,markers[i].setCenter, markers[i].listeners);
                    }
                }
            }
        }
...
*/
    addMarker : function(point, marker, clear,icon, center, listeners){
        ...
        if (marker.infoWindow){
           ...
           // marker.infoWindow.open(this.getMap());
        }
    ...
    createInfoWindow : function(inwin, point, marker){
    	...
    	//  infoBubble = new InfoBubble({
          ...
          //  });
            ...
            infoBubble2.open(this.getMap(), marker);
            //infoBubble2.open();
...
//        	
//        	
//            //google.maps.event.addListener(marker, 'start', function(){
//             //   me.hideAllInfoWindows();
//                infoWindow.open(me.getMap());
//           // });
//        }
//        
       this.cache.infowindow.push(infoBubble2);
//
//        return infoWindow;
		return infoBubble2;

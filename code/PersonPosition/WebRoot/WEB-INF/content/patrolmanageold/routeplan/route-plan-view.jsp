...
//	function drow_line(data_list,index){//没有动画时的调用方法
//		var start_point = data_list[index];
//		var end_point = data_list[index+1];
//		var lineWidth = Math.sqrt(Math.pow((Number(start_point.location_x)-Number(end_point.location_x)),2)+
//					Math.pow((Number(start_point.location_y)-Number(end_point.location_y)),2));
...
//		var line_center_left = (Number(start_point.location_x) + Number(end_point.location_x) - lineWidth)/2+2;//2为route_div的高度的一半
//		$('#route_plan_route_line').append('<div class="route_div" style="top: '+line_center_top+'px; left: '+line_center_left+'px;width: '+lineWidth+'px;"></div>');
//		var lineObj = $('#route_plan_route_line').find('.route_div:last');
//		lineObj.css("transform", "rotate("+angle+"deg)");
...
//	var routePlanTask;
//	function drowRoutePlan(locations,index){//描点的画线方式
...
//			routePlanTask = new Ext.util.DelayedTask(function(){
...
//					insertRoutePlanPoint(locations[index+1]);
////					$('#route_plan_point').append(getRoutePointHtml(locations[index+1]));
////					$("#iPicture_route_plan").iPictureInsert($('#route_plan_point').find('.ip_tooltip:last'));
//				}else{
//					var tooltips_add = '<div class="route" style="top: ' + positionY + 'px; left: ' + positionX +'px;"></div>';
//					$('#route_plan_route_line').append(tooltips_add);
//				}
//				times = times+1;
//				if(currentLengthXY < lengthXY){
//					routePlanTask.delay(0);
//				}else{
//					drowRoutePlan(locations,index+1);
//				}
//			});
//			routePlanTask.delay(0);
//		}else{
...
//	}
	...
	function insertRoutePlanPoint(point_obj){
		var routePointHtml = '<div data-point-id="'+point_obj.id+'" class="ip_tooltip ip_img32" ' +
			...
			'<p>路线点信息<hr>' +
			'名称：'+point_obj.name +
			'<br>有效范围：'+point_obj.effective_range +
			'<br>有效开始时间：'+point_obj.effective_start_time +
			'<br>有效结束时间：'+point_obj.effective_end_time +
			'<br>添加时间：'+point_obj.add_date +
			'</p></div>';
		$('#route_plan_point').append(routePointHtml);
		var currentPoint = $('#route_plan_point').find('.ip_tooltip:last');
		$("#iPicture_route_plan").iPictureInsert(currentPoint);
		//插入范围效果
	...
	function removeRoutePlanPoint(pointId){
		Ext.Msg.confirm('确认', '确定要删除该检查点？',function(btn, text){
			if (btn == 'yes') {
				Ext.Msg.wait('删除数据中....', '提示');
				Ext.Ajax.request({
					url:'patrolmanage/routeplan/delete-route-plan-point.action',
...
//				if (btn == 'yes') {		
					showRoutePointForm("",locationX,locationY);
//				}
//			});
		}
	}
	function showRoutePointForm(id,locationX,locationY){
		Ext.Msg.wait('加载数据中....', '提示');
		Ext.Ajax.request({
			async : false,
			url:'patrolmanage/routeplan/add-or-modify-route-point.action',
			params : {id:id,routeId:tree.getSelectionModel().getSelectedNode().id,locationX:locationX,locationY:locationY},
			success : function(xhr){
				Ext.Msg.hide();
				var routePointInfoWin = eval(xhr.responseText);
				routePointInfoWin.show();
			},
			failure:function(xhr){
				Ext.Msg.hide(); 

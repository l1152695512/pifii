	...
	var addPerson= new Ext.Action({
        ...
        handler: function(){
        	if(currentNodeId == rootNodeId){
        		Ext.Msg.alert("提示","请先点击左边的具体路线，再进行添加！");
        		return;
        	}
        	Ext.Msg.wait('获取人员列表....', '提示');
			Ext.Ajax.request({
				async : false,
				url:'patrolmanage/routeassign/add-assign-person.action',
				params : {routeId:currentNodeId},
		    ...
		    Ext.Msg.confirm('确认', '确定要删除选中的人员？',
				function(btn, text) {
					if (btn == 'yes') {
						...
						Ext.Ajax.request({
				       		url:'patrolmanage/routeassign/delete-assign-person.action',
				       		params: {ids:params},
				            success : function(xhr) {
				            	Ext.Msg.hide(); 
				            	assigned_person_store.reload();

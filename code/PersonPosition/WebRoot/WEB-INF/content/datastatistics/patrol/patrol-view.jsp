...
(function(){
	var pageLimit = 20;
	var patrol_storeFields = ['date','person_name','community_name','map','route_name','position_date',
					'person_id','phone','route_time_id','start_time','end_time','is_done'];
		 ...
		 actions:[{
			...
			callback:function(grid, record, action, row, col){
				...
				Ext.Ajax.request({
					...
					params:{personName:record.data.person_name,routeName:record.data.route_name,communityName:record.data.community_name,
								date:record.data.date,personId:record.data.person_id,phone:record.data.phone,routeTimeId:record.data.route_time_id,mapPath:record.data.map},
	...
	var patrol_gridColumns = [
    	new Ext.grid.RowNumberer({
			...
			renderer : function(value, metadata, record, rowIndex) {
				return patrol_store.lastOptions.params.start + 1 + rowIndex;
			}
		}),{
            header: "时间",
            dataIndex: 'date',
    ...
    var exportExcel = new Ext.Action(Ext.apply({
    	...
    	handler: function(){
    		...
    		if(params.routeId != ''){
    			params.routeName = route_store.getById(params.routeId).get("name");
    		}
    		var paramsEncode = Ext.urlEncode(params);
    		console.debug(paramsEncode);
    		var url = "datastatistics/patrol/export-excel.action?" + paramsEncode;
...
//		forceSelection : true, 
		...
		store : route_store, 	
		listeners : {
			'select' : function(route,record,index ){
				point_show.clearValue();
				point_store.reload({params : {
					pointId : route.value
				}});
			}
		}
	});
	var point_store = new Ext.data.JsonStore({ 
		url:'datastatistics/patrol/get-points.action',
		fields : ['id','name'], 
		root : "values"
	});
	point_store.addListener("load", function(){ 
		point_store.insert(0, new Ext.data.Record({ 
			'id' : "",
			'name' : '全部'
		})); 
		point_show.setValue(this.getAt(0).get('id'));
	}); 
	point_store.load();
	var point_show = new Ext.form.ComboBox( { 
		fieldLabel : '路&nbsp;&nbsp;线&nbsp;&nbsp;点', 
		mode : 'local', 
		triggerAction : 'all', 
//		selectOnFocus : true, 
...
//        split:true,
        ...
        items: [{
            ...
            name:'name',
            maxLength:15
        },community_show,route_show,point_show],//,is_done_show
        buttons:[{
         	text:'查询',
         	handler: function () {
			 	patrol_store.reload();

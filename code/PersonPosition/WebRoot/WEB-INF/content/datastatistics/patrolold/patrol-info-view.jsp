...
(function(){
	...
	function load_route(){
		Ext.Msg.wait('正在加载巡更点....', '提示');
	   	Ext.Ajax.request({
       		url : 'datastatistics/patrol/patrol-info-data.action',
       		params : {personId : '${personId}',routeId : '${routeId}',date : '${date}'},
	...
	function insertRoutePlanPoint(point_obj){
		...
		if(point_obj.is_done == "1"){//该检查点完成
			...
			tip = "<span style='color:blue'>完成</span>";
		}else{
			background = "rgba(0,0,0, 0.5)";
			tip = "<span style='color:red'>未完成</span>";
		}
		...
		var currentPoint = $('#patrol_info_point').find('.ip_tooltip:last');
		$("#patrol_info_map").iPictureInsert(currentPoint);
		//插入范围效果
		var range = Number(point_obj.effective_range);
		currentPoint.prepend('<div class="route-plan-point-range" style="background: '+background+';width:'+range+'px;height:'+range+'px;top:-'+range*0.46+'px;left:-'+range*0.46+'px"></div>');
	}
	...
	var patrol_info_win = new Ext.Window({
		title: '${routeName}&nbsp;&nbsp;>&nbsp;&nbsp;${personName}&nbsp;&nbsp;>&nbsp;&nbsp;${date}',
		layout : 'fit',
        border:false,
        modal :true,
        layout: 'fit',

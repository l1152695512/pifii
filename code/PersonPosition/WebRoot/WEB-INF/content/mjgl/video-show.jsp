            ...
            success : function(xhr) {
            	...
            	if(dataList.length == 0){
	            	...
	            	for(var i=0;i<dataList.length;i++){
	            		if(dataList[i].videoDate != currentDate){
							currentDate = dataList[i].videoDate;
						...
						$("#photo_div_outer").children().last().find(".pics").append(
								'<div class="pic">' +
									'<embed src="'+dataList[i].videoContent+'" type="video/quicktime" scale="tofit" controller="true" autostart="false"> '+
//									'<embed src="'+dataList[i].videoContent+'" autostart="true" loop="true" width="400" height="350" >' +
//							 		'<video controls="controls">' +
//							 			'<source id="videoSource" src="'+dataList[i].videoContent+'" type="video/mp4">' +
//										'你的浏览器不支持视频播放！' +
//									'</video>' +
//									'<video controls preload><source src="'+dataList[i].videoContent+'"></source></video>' +
							 		'<div>'+dataList[i].videoTime+'</div>' +
								'</div>');
					}
					...
					$("#photo_div_outer").find(".pic").find("embed").css({"height":"300px","width":"250px"});

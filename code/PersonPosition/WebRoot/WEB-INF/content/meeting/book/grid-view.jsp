...
(function(){
	var tabTitle = "会议预约";
	var gridDataUrl= 'meeting/book/grid-data.action';
	...
	var orderAction=new Ext.Action({
		text : '预约向导',
		iconCls : 'icon-guide',
		handler : function() {
			Ext.Ajax.request({
				url:'meeting/book/order-form.action',
	...
	var myOrderAction=new Ext.Action({
		text : '我的预约',
		iconCls : 'icon-book',
		handler : function() {
			Ext.Ajax.request({
				url:'meeting/book/order-view.action',
				params:{userName:'${userName}'},
				success:function(xhr){
					var newForm=eval(xhr.responseText);
					if(newForm==undefined||newForm=='undefined')return;
		...
		cellclick:function(grid, rowIndex, columnIndex, e){	
			if(columnIndex!=0 && columnIndex<=6){
				...
				if(temp=='空闲'){
					...
					Ext.Msg.alert('提示',columnIndex+'号会议室，时间段['+rec.id+']，已被['+temp+']预约');
				}
			}
		} 
	};
	...
	var grid=new Ext.grid.GridPanel({
		columns:gridColumns,
		store:store,
		tbar:[queLabel,queInfo,'&nbsp;&nbsp;',queAction,'-',orderAction,'-',myOrderAction],
		stripeRows:true,

...
(function(){
	var pageLimit = 20;
	var gridDataUrl = "sys/usergroup/action/grid-data.action";
	var newFormUrl = "sys/usergroup/action/select-view.action";
	//var newFormUrl = "sys/usergroup/action/new-form.action";
	var rowId = 'sugiad';
	var storeFields = [
            'sysActionName',
    ...
    var delAction =  new Ext.Action({
        ...
        handler: function(){
			...
			if(params.length == 0){
	           ...
	           Ext.Ajax.request({
				   url: 'sys/usergroup/action/delete-data.action',
				   success: reloadFn,
				   failure: reloadFn,
				   params: { ids: params }
				});
        	  ...
        	  //永远不执行
			   if(columnIndex < 0){
			    	var rec = grid.store.getAt(rowIndex);
	       			Ext.Ajax.request({
			    		url:"sys/usergroup/action/edit-form.action",
    ...
    store.setDefaultSort('sugiad', 'desc');

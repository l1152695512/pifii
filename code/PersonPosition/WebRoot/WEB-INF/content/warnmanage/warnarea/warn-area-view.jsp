...
//            		insert_warn_area_point(dataList[i]);
            		var areaPointHtml = '<div data-point-id="'+dataList[i].id+'" class="ip_tooltip ip_img32" ' +
							'style="top: '+dataList[i].location_y+'px; left: '+ dataList[i].location_x+
							'px;"><div class="button route-plan-point"></div></div>';
					$('#warn_area_point').append(areaPointHtml);
...
//		});
	...
	Ext.add_warn_area_point = function(event){
		...
		if(selectNode && selectNode.isLeaf()){
			Ext.Msg.confirm('确认', '确定要在此处添加区域点？',function(btn, text){
				if (btn == 'yes') {
					...
					Ext.Ajax.request({
						...
						params : {areaId:tree.getSelectionModel().getSelectedNode().id,locationX:locationX,locationY:locationY},
						success : function(xhr){
							Ext.Msg.hide();
							load_warn_area_points(tree.getSelectionModel().getSelectedNode().id);
						},
			...
			Ext.Msg.alert("提示","请先选择或者创建区域，然后添加区域点！");
		}

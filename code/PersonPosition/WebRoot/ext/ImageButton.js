...
Ext.extend(Ext.ux.ImageButton, Ext.Button, {
	onRender: function(ct, position) {
		...
		var tplHTML = '<div><img src="{imgPath}" border="0" width="{imgWidth}" height="{imgHeight}" alt="{tooltip}" title="{title}" style="cursor: {cursor};"/> {imgText:htmlEncode}<br/><br/></div>';
	...
	disable: function(newImgPath) {
		...
		if (replaceImgPath) this.el.dom.firstChild.src = replaceImgPath;
		this.disabled = true;
	},

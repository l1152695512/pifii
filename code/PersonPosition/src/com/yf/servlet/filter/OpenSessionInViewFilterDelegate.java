...
public class OpenSessionInViewFilterDelegate extends OpenSessionInViewFilter {
	...
	private boolean filterURI(String uriWithoutContextPath){
		for (int i = 0; i < exclude.length; i++){
			if(logger.isDebugEnabled()){
				logger.debug("test exclude, pattern:"+exclude[i]+",uri:"+uriWithoutContextPath);
			}
			if(antPathMatcher.match(exclude[i],uriWithoutContextPath)){
				if(logger.isDebugEnabled()){
					logger.debug("excluded, pattern:"+exclude[i]+",uri:"+uriWithoutContextPath);
				}
				return false;
			}
		}
		...
		for (int i = 0; i < mappings.length; i++) {
			if(logger.isDebugEnabled()){
				logger.debug("matching,pattern:"+mappings[i]+",uri:"+uriWithoutContextPath);
			}
	...
	public static void main(String[] args){
		...
		Debug.println(m.match("/**/main.action", "/admin/js/main.action"));
	}

...
import com.yf.servlet.ValidateCodeServlet;
...
public class ValidateCodeCheckFilter implements Filter {
	...
	public void doFilter(ServletRequest request, ServletResponse response,
		...
		String sessionValidateCode = (String)httpRequest.getSession().getAttribute(ValidateCodeServlet.VALIDATE_CODE_SESSION_NAME);
		String validateCode = request.getParameter(VALIDATE_CODE_FIELD_NAME);
		if(httpRequest.getRequestURI().equals(httpRequest.getContextPath() + formLoginUrl)){
			if(sessionValidateCode != null && sessionValidateCode.equals(validateCode)){
				chain.doFilter(request, response);
				return ;
			}else{
				((HttpServletResponse)response).sendRedirect(httpRequest.getContextPath()+failRedirectUrl);
				return;
			}
		}
		chain.doFilter(request, response);

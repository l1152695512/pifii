...
import com.yf.exception.UpLoadImgFormatException;
...
public class UploadServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            if(itemList != null){
            	...
            	while(it.hasNext()){
            		...
            		if(chkImgName(item.getName())){
	            		if(!item.isFormField()){
	            			  String uploadPath = this.getServletContext().getRealPath("/temp/"+req.getSession().getId());
	                          ...
	                          fileName = RandomStringUtils.random(18,true,true)+"."+suffix;
	                          ...
	                          try {
								...
								e.printStackTrace();
								req.setAttribute("uploadError", "上传错误：找不到"+uploadPath+"请检查！");
							} catch (Exception e) {
								e.printStackTrace();
								//文件写入磁盘异常，待处理。。。。
							}
			...
			req.getRequestDispatcher("/Main/backend/upload.jsp").forward(req, resp);
		}
	}
	
	private static boolean chkImgName(String imgName){
		String tempName = imgName.substring(imgName.length()-3, imgName.length());
		if(tempName.equalsIgnoreCase("jpg") || tempName.equalsIgnoreCase("gif") || tempName.equalsIgnoreCase("png") || tempName.equalsIgnoreCase("jpeg")){
			return true;
		}else{
			return false;
		}

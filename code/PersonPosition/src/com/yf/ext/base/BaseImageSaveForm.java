...
public abstract class BaseImageSaveForm extends ActionSupport implements ServletRequestAware{
	...
	public String execute() throws Exception {
		...
		if(StringUtils.isNotBlank(uploadFileName)){
			File file = new File(userTempStore.getFile(request.getSession().getId()),uploadFileName);
			...
			if(!uploadFileName.equals(oldFileName)){//文件名与旧图不一样，表示有新图片,需要保存。
				...
				File dulplicatFile = new File(fileStore.getRootFile(),uploadFileName);

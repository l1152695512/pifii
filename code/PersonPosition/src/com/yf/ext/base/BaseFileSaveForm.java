...
public abstract class BaseFileSaveForm extends ActionSupport implements
		ServletRequestAware {
...
//        ActionContext.getContext().getSession().put("fileNameForAdd",fileName);//保存的文件
		...
		if (StringUtils.isNotBlank(uploadFileName)) {
			...
			if(uploadFileName.equals("fileNameForAdd")){
				uploadFileName=(String)ActionContext.getContext().getSession().get("fileNameForAdd");
				file = new File(userTempStore.getFile(request.getSession().getId()), uploadFileName);
				File file2= new File(userTempStore.getFile(request.getSession().getId()), this.editId+uploadFileName.substring(14));

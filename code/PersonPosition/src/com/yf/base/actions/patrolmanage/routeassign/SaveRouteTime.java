...
public class SaveRouteTime extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		if(StringUtils.isNotBlank(id)){
			String sqlOldData = "select start_time,end_time from bp_fine_route_time_tbl where id = '"+id+"'";
			...
			if(dataList.size() > 0){
				...
				if(!oldStartTime.equals(startTime) || !oldEndTime.equals(endTime)){//修改了路线时间，则删除路线点的时间段设置
					String sqlDelete = "delete from bp_fine_route_point_time_tbl where route_time_id = '"+id+"'";
					dbhelper.delete(sqlDelete);
				}
			}
			sql.append("update bp_fine_route_time_tbl set name=?,start_time=?,end_time=?,is_used=? where id=? ");
			...
			sql.append("insert into bp_fine_route_time_tbl(id,route_id,name,start_time,end_time,is_used,add_date) ");

...
//	private int columnsNum = 0;
	private String[] headers = new String[]{"日期","人员","人员类型","小区","路线","位置","电话","开始时间","结束时间","完成进度"};
	private String sheetName;
	
	@Override
	public String execute() throws Exception {
		sheetName = "巡更数据("+startDate+" - "+endDate+").xls";
	...
	private List<?> getExcelData(){
		...
		if(CommonSearch.initSearchTable(dbhelper, startDate, endDate)){
			...
			sql.append("r.name as route_name,rp.name as route_point_name,DATE_FORMAT(prl.upload_time,'%Y-%m-%d %H:%i:%s') as position_date,p.phone,");
			...
			sql.append("join bp_fine_route_point_tbl rp on (");
			if(!StringUtils.isBlank(pointId)){
				sql.append("rp.id = '"+pointId+"' and ");
			}
			sql.append("rp.route_id = r.id) ");
			...
			if(StringUtils.isNotBlank(startDate)){
				sql.append("date(prl.upload_time) >= '"+startDate+"' and ");
			}
			if(StringUtils.isNotBlank(endDate)){
				sql.append("date(prl.upload_time) < '"+endDate+"' and ");
			}

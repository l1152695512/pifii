...
public class PatrolData extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		if(CommonSearch.initSearchTable(dbhelper, startDate, endDate)){
			...
			dataSql.append("select date,community_name,map,route_name,start_time,end_time,person_name,position_date,person_id,route_id,if(min(is_done) = 1,1,0) is_done ");
			StringBuffer commonSql = new StringBuffer();
			commonSql.append("from (select sd.date,r.name route_name,po.name point_name,po.effective_start_time as start_time,po.effective_end_time as end_time,DATE(lh.date) position_date,pe.name person_name,c.name community_name,c.map,rp.person_id,rp.route_id ");
			...
			commonSql.append("date(lh.date) = sd.date and rp.person_id = lh.personId) ");
			commonSql.append("group by rp.person_id,po.id,sd.date ");
			commonSql.append(") a ");
			commonSql.append("group by date,route_id,person_id ");

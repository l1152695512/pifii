...
public class GridData extends ActionSupport {
	...
	public String execute() throws Exception {	
		...
		for(int i=0;i<strDate.length-1;i++){
			...
			for(int j=1;j<9;j++){
				String sql="select CHARGE from BP_MEETING_TBL where START_TIME<=to_date('"+queryInfo.substring(0, 10)+" "+strDate[i]+"','yyyy-MM-dd HH24:mi:ss') and END_TIME>=to_date('"+queryInfo.substring(0, 10)+" "+strDate[i+1]+"','yyyy-MM-dd HH24:mi:ss') and M_ROOM='"+j+"'";
				RowSet rs=dbhelper.select(sql);
				if(rs.next()){
					strArray[i][j]=rs.getString("CHARGE");
				}else {
					strArray[i][j]="空闲";
				}
			}

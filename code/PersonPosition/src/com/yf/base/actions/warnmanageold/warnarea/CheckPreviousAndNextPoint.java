...
public class CheckPreviousAndNextPoint extends ActionSupport {
	...
	public String execute() throws Exception {
		if(StringUtils.isBlank(previousPoint) && StringUtils.isBlank(nextPoint)){
			...
			sql.append("select count(*) count from bp_fine_area_point_tbl where area_id = '");
			...
			StringBuffer sql = new StringBuffer();
			sql.append("select id from bp_fine_area_point_tbl where area_id = '"+warnAreaId+"' and (previous_point_id is null or trim(previous_point_id) = '') ");
			List<Map<String,Object>> dataList = (List<Map<String, Object>>) dbhelper.getMapListBySql(sql.toString());
			if(dataList.size()>0){
				Map<String,Object> data = dataList.get(0);
			...
			StringBuffer sql = new StringBuffer();
			sql.append("select id from bp_fine_area_point_tbl where area_id = '"+warnAreaId+"' and (next_point_id is null or trim(next_point_id) = '') ");
			List<Map<String,Object>> dataList = (List<Map<String, Object>>) dbhelper.getMapListBySql(sql.toString());
			if(dataList.size()>0){
				Map<String,Object> data = dataList.get(0);
			...
			StringBuffer sql = new StringBuffer();
			sql.append("select next_point_id from bp_fine_area_point_tbl where id = '");
			...
			if(dataList.size()>0){
				...
				if(!nextPointId.equals(nextPoint)){//判断要插入的点所在的两点之间有没有其他的点
					...
					sql2.append("select next_point_id from bp_fine_area_point_tbl where id = '");
					sql2.append(id);
					sql2.append("' ");

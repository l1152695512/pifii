	...
	public String execute() throws Exception {
		...
		try {
			...
			if(StringUtils.isNotBlank(id)){//更新
				sql.append("update bp_fine_area_point_tbl ");
				...
				sql.append("insert into bp_fine_area_point_tbl(id,area_id,name,location_x,location_y,");
				sql.append("previous_point_id,next_point_id,add_date) ");
				sql.append("VALUES(?,?,?,?,?,?,?,now()) ");
				preparedStatement = conn.prepareStatement(sql.toString());
				String id = UUID.randomUUID().toString().replaceAll("-", "");
				preparedStatement.setString(1, id);
				preparedStatement.setString(2, warnAreaId);

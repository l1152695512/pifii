...
public class GetRoutePointsList extends ActionSupport {
	...
	public String execute() throws Exception {
		String sql = "select id,name,previous_point_id,next_point_id from bp_fine_route_point_tbl where route_id='"+routeId+"' order by previous_point_id ";
		...
		int removeRoutePointIndex = -1;
		if(dataList.size()>0){
			sortDataList.add(dataList.get(0));
			System.err.println(dataList.get(0).get("name").toString());
			if(dataList.get(0).get("id").toString().equals(id)){
				removeRoutePointIndex = 0;
			}
			dataList.remove(0);
		}
		while(dataList.size()>0){
			...
			if(removeRoutePointIndex == -1 && nextPoint.get("id").toString().equals(id)){
				removeRoutePointIndex = sortDataList.size();
			}
			sortDataList.add(nextPoint);
		}
		if(removeRoutePointIndex != -1){
			sortDataList.remove(removeRoutePointIndex);
		}
	...
	private Map<String,Object> findPoint(List<Map<String,Object>> dataList,String pointId){
		...
		for(int i=0;i<dataList.size();i++){
			if(dataList.get(i).get("id").equals(pointId)){
				...
				System.err.println(findData.get("name").toString());
				dataList.remove(i);
				return findData;
			}
		}

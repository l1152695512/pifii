...
import com.yinfu.routersyn.marker.VideoMarker;
...
public class VideoTask extends BaseTask {
	...
	public static String marker = "video";//marker字段放到task中，统一使用
	private static String THIS_APP_FOLDER = "mb/video";
	public static String IMAGE_FOLDER = THIS_APP_FOLDER+"/logo";
	public static String FILE_FOLDER = THIS_APP_FOLDER+"/v";
	private String htmlFolder;
	private String imageFolder;
	private Object resId;
	private Object shopId;
	
	public VideoTask(Object shopId,Object resId,Record taskInfo,List<Record> publishDevices) {//用于发布、删除或者修改
...
//		initPublishDevices(publishDevices);
	...
	public VideoTask(Object shopId,String baseFolder) {//仅用于复制资源文件，如图片和html，用于发布应用或者新绑定盒子
...
//		this.shopId = shopId;
	...
	private void init(){
		htmlFolder = baseFolder + File.separator + "mb" + File.separator + "video";
...
//	}
	...
	protected boolean copyRes(List<Record> otherSynTask){
		return copyFile(otherSynTask) && VideoMarker.execute(shopId,htmlFolder);//生成html;
	} 
	...
	public static Map<String,List<File>> synRes(Object shopId,Object resId,List<String> sqls,Record taskInfo,
		...
		BaseTask index = new VideoTask(shopId,resId,taskInfo,publishDevices);
	...
	private boolean copyFile(List<Record> otherSynTask){
		...
		if(null == resId){//整体图片资源同步
			sqls.append("select icon img_path,link file_path,delete_date,md5,name from bp_video where delete_date is null and status=1 ");
			...
			sqls.append("select icon img_path,link file_path,delete_date,md5,name from bp_video where id = '"+resId+"'");//这里要确保删除资源文件为软删除，不然这个记录也不会被查找出来
		}
		if(sqls.length() > 0){
...
//							logger.info("复制了video图片文件："+file.getAbsolutePath());
						}else{
							logger.warn("video图片文件不存在！"+file.getAbsolutePath());
						}
					} catch (IOException e) {//这里出现异常任务可继续
						e.printStackTrace();
						logger.warn("复制video图片文件异常！"+file.getAbsolutePath(), e);
					}
					otherSynTask.add(new Record().set("operate_type", "add").set("res_url", fileRec.get("file_path"))
							...
							.set("task_desc", "同步视频【"+fileRec.get("name")+"】"));
				}else{//删除的资源

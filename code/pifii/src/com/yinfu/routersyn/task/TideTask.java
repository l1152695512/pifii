...
import com.yinfu.routersyn.marker.TideMarker;
 ...
 *
 */
public class TideTask extends BaseTask {
	private static Logger logger = Logger.getLogger(TideTask.class);
	public static String marker = "app_tide";//marker字段放到task中，统一使用
	private static String THIS_APP_FOLDER = "mb/mall";
	public static String IMAGE_FOLDER = THIS_APP_FOLDER+"/images";
	private String htmlFolder;
	private String imageFolder;
	
	public TideTask(Object shopId,Record taskInfo,List<Record> publishDevices) {
		super(taskInfo,shopId,publishDevices);
//		this.shopId = shopId;
		init();
	}
	
	public TideTask(Object shopId,String baseFolder) {
...
//		this.shopId = shopId;
	...
	protected boolean copyRes(List<Record> otherSynTask){
		return copyFile() && TideMarker.execute(shopId, htmlFolder);//生成html;
	} 
	...
	public static Map<String,List<File>> synRes(Object shopId,List<String> sqls,Record taskInfo,
    	...
    	if(SynUtils.checkShopPublished(shopId)){
    		...
    		BaseTask index = new TideTask(shopId,taskInfo,publishDevices);
	...
	private boolean copyFile(){
		...
		//复制数据文件
		List<Record> files = Db.find("select img from bp_tide where shop_id=? and delete_date is null ", new Object[]{shopId});
...
//					logger.info("复制了tide文件："+iconFile.getAbsolutePath());
				}else{
					logger.warn("tide文件不存在！"+iconFile.getAbsolutePath());
				}
			} catch (IOException e) {//这里出现异常任务可继续
				e.printStackTrace();
				logger.warn("复制tide文件异常！"+iconFile.getAbsolutePath(), e);
			}
		}
		return true;
	}

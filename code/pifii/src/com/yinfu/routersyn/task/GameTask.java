...
import com.yinfu.routersyn.marker.GameMarker;
 ...
 *
 */
public class GameTask extends BaseTask {
	private static Logger logger = Logger.getLogger(GameTask.class);
	public static String marker = "game";//marker字段放到task中，统一使用
	private static String THIS_APP_FOLDER = "mb/games";
	public static String IMAGE_FOLDER = THIS_APP_FOLDER+"/logo";
	public static String FILE_FOLDER = THIS_APP_FOLDER+"/f";
	private String htmlFolder;
	private String imageFolder;
	private Object resId;
	
	public GameTask(Object shopId,Object resId,Record taskInfo,List<Record> publishDevices) {//用于发布、删除或者修改
...
//		initPublishDevices(publishDevices);
	...
	public GameTask(Object shopId,String baseFolder) {//仅用于复制资源文件，如图片和html，用于发布应用或者新绑定盒子
...
//		this.shopId = shopId;
	...
	private void init(){
		htmlFolder = baseFolder + File.separator + "mb" + File.separator + "games";
...
//	}
	...
	protected boolean copyRes(List<Record> otherSynTask){
		return copyFile(otherSynTask) && GameMarker.execute(shopId,htmlFolder);//生成html;
	} 
	...
	public static Map<String,List<File>> synRes(Object shopId,Object resId,List<String> sqls,Record taskInfo,
		...
		BaseTask index = new GameTask(shopId,resId,taskInfo,publishDevices);
	...
	private boolean copyFile(List<Record> otherSynTask){
		...
		if(null == resId){//整体图片资源同步
			sqls.append("select icon img_path,link file_path,delete_date,md5,name from bp_game where delete_date is null and status=1 ");
			...
			sqls.append("select icon img_path,link file_path,delete_date,md5,name from bp_game where id = '"+resId+"'");//这里要确保删除资源文件为软删除，不然这个记录也不会被查找出来
		}
		if(sqls.length() > 0){
...
//							logger.info("复制了game图片文件："+file.getAbsolutePath());
						}else{
							logger.warn("game图片文件不存在！"+file.getAbsolutePath());
						}
					} catch (IOException e) {//这里出现异常任务可继续
						e.printStackTrace();
						logger.warn("复制game图片文件异常！"+file.getAbsolutePath(), e);
					}
					otherSynTask.add(new Record().set("operate_type", "add").set("res_url", fileRec.get("file_path"))
							...
							.set("task_desc", "同步游戏【"+fileRec.get("name")+"】").set("res_type", "archive"));
				}else{//删除的资源

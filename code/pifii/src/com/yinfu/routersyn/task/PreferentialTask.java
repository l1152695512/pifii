...
import com.yinfu.routersyn.marker.PreferentialMarker;
 ...
 *
 */
public class PreferentialTask extends BaseTask {
	private static Logger logger = Logger.getLogger(PreferentialTask.class);
	public static String marker = "app_preferential";//marker字段放到task中，统一使用
	private static String THIS_APP_FOLDER = "mb/mall";
	public static String IMAGE_FOLDER = THIS_APP_FOLDER+"/images";
	private String htmlFolder;
	private String imageFolder;
	
	public PreferentialTask(Object shopId,Record taskInfo,List<Record> publishDevices) {
		super(taskInfo,shopId,publishDevices);
//		this.shopId = shopId;
		init();
	}
	
	public PreferentialTask(Object shopId,String baseFolder) {
...
//		this.shopId = shopId;
	...
	protected boolean copyRes(List<Record> otherSynTask){
		return copyFile() && PreferentialMarker.execute(shopId, htmlFolder);//生成html;
	} 
	...
	public static Map<String,List<File>> synRes(Object shopId,List<String> sqls,Record taskInfo,
    	...
    	if(SynUtils.checkShopPublished(shopId)){
    		...
    		BaseTask index = new PreferentialTask(shopId,taskInfo,publishDevices);
	...
	private boolean copyFile(){
		...
		//复制数据文件
		List<Record> files = Db.find("select img from bp_preferential where shop_id = ? and delete_date is null ", new Object[]{shopId});
		Iterator<Record> ite = files.iterator();
		while(ite.hasNext()){
			Record img = ite.next();
			File iconFile = new File(SynUtils.getResBaseFloder()+File.separator+(img.getStr("img").replaceAll("/", File.separator+File.separator)));
			try {
				if(iconFile.exists()){
					FileUtils.copyFileToDirectory(iconFile, new File(imageFolder));
//					logger.info("复制了Preferential文件："+iconFile.getAbsolutePath());
				}else{
					logger.warn("Preferential文件不存在！"+iconFile.getAbsolutePath());
				}
			} catch (IOException e) {//这里出现异常任务可继续
				e.printStackTrace();
				logger.warn("复制Preferential文件异常！"+iconFile.getAbsolutePath(), e);
			}
		}
		return true;
	}

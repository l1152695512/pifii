...
import com.yinfu.routersyn.marker.FunnyMarker;
 ...
 *
 */
public class FunnyTask extends BaseTask {
	private static Logger logger = Logger.getLogger(FunnyTask.class);
	public static String marker = "app_funny";//marker字段放到task中，统一使用
	private static String THIS_APP_FOLDER = "mb/mall";
	public static String IMAGE_FOLDER = THIS_APP_FOLDER+"/images";
	private String htmlFolder;
	private String imageFolder;
	
	public FunnyTask(Object shopId,Record taskInfo,List<Record> publishDevices) {
		super(taskInfo,shopId,publishDevices);
//		this.shopId = shopId;
		init();
	}
	
	public FunnyTask(Object shopId,String baseFolder) {
...
//		this.shopId = shopId;
	...
	protected boolean copyRes(List<Record> otherSynTask){
		return copyImg() && FunnyMarker.execute(shopId, htmlFolder);//生成html;
	} 
	...
	public static Map<String,List<File>> synRes(Object shopId,List<String> sqls,Record taskInfo,
    	...
    	if(SynUtils.checkShopPublished(shopId)){
    		...
    		BaseTask index = new FunnyTask(shopId,taskInfo,publishDevices);
	...
	private boolean copyImg(){
		...
		List<Record> imgs = Db.find("select img from bp_funny where shop_id=? and delete_date is null ", new Object[]{shopId});
...
//					logger.info("复制了funny图片："+iconFile.getAbsolutePath());
				}else{
					logger.warn("funny图片不存在！"+iconFile.getAbsolutePath());
				}
			} catch (IOException e) {//这里出现异常任务可继续
				e.printStackTrace();
				logger.warn("复制funny图片异常！"+iconFile.getAbsolutePath(), e);
			}
		}
		return true;
	}

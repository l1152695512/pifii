...
import com.yinfu.routersyn.marker.ApkMarker;
...
public class ApkTask extends BaseTask {
	private static Logger logger = Logger.getLogger(ApkTask.class);
	public static String marker = "apk";//marker字段放到task中，统一使用
	private static String THIS_APP_FOLDER = "mb/app";
	...
	public ApkTask(Object shopId,Object resId,Record taskInfo,List<Record> publishDevices) {//用于发布、删除或者修改，会生成更新包task
...
//		initPublishDevices(publishDevices);
	...
	public ApkTask(Object shopId,String baseFolder) {//仅用于复制资源文件，不用于生成更新包，如图片和html，用于发布应用或者新绑定盒子
...
//		this.shopId = shopId;
	...
	private void init(){
		htmlFolder = baseFolder + File.separator + "mb" + File.separator + "app";
...
//	}
	...
	protected boolean copyRes(List<Record> otherSynTask){
		return copyFile(otherSynTask) && ApkMarker.execute(shopId,htmlFolder);//生成html;
	} 
	...
	public static Map<String,List<File>> synRes(Object shopId,Object resId,List<String> sqls,Record taskInfo,
		...
		BaseTask index = new ApkTask(shopId,resId,taskInfo,publishDevices);
	...
	private boolean copyFile(List<Record> otherSynTask){
		...
		if(null == resId){//整体图片资源同步
			sqls.append("select icon img_path,link file_path,delete_date,md5,name from bp_apk where delete_date is null and status=1 ");
			...
			sqls.append("select icon img_path,link file_path,delete_date,md5,name from bp_apk where id = '"+resId+"'");//这里要确保删除资源文件为软删除，不然这个记录也不会被查找出来
		}
		if(sqls.length() > 0){
...
//							logger.info("复制了apk图片文件："+file.getAbsolutePath());
						}else{
							logger.warn("apk图片文件不存在！"+file.getAbsolutePath());
						}
					} catch (IOException e) {//这里出现异常任务可继续
						e.printStackTrace();
						logger.warn("复制apk图片文件异常！"+file.getAbsolutePath(), e);
					}
					otherSynTask.add(new Record().set("operate_type", "add").set("res_url", fileRec.get("file_path"))
							...
							.set("task_desc", "同步安装包【"+fileRec.get("name")+"】"));
				}else{//删除的资源

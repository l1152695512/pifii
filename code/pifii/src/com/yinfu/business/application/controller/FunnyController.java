...
import com.yinfu.business.application.model.Funny;
 ...
 *最新优惠
 */
@ControllerBind(controllerKey = "/business/app/funny", viewPath = "/page/business/application/funny")
public class FunnyController extends Controller<Record> {
	private static final String PREVIEW_PATH = "upload" + File.separator + "image" + File.separator + "funny" + File.separator;//图片保存路径
	//@formatter:off 
	/**
	 * Title: show
	 * Description:段子页面
	...
	public void listData(){
		...
		renderJson(Funny.dao.funnyList(shopId, name));
	}
	//@formatter:off 
	/**
	 * Title: update
	...
	public void update(){
		...
		Funny funny = Funny.dao.findById(id);
		setAttr("funny", funny);
		renderJsp("funnyAdd.jsp");
	}
	//@formatter:off 
	/**(non-Javadoc)
	 * Title: delete
	...
	public void delete(){
		...
		boolean isSuccess = Db.tx(new IAtom(){public boolean run() throws SQLException {
			...
			return  Db.update("update bp_funny set delete_date=now() where id=? ", new Object[]{getPara("id")})>=1
					&& synFile(deleteRes,routerDeleteRes,"删除了",getPara("id"));
		}});
		if(isSuccess){
			SynUtils.deleteRes(deleteRes.get("success"));
			try{
				Record rec = Db.findFirst("select shop_id from bp_funny where id=? ", new Object[]{getPara("id")});
				boolean success = DataSynUtil.addTask(rec.get("shop_id").toString(), "funny", getPara("id"), "2");
//				if(success){//这里不能基于succes判断是否执行下面代码，坑
					PageUtil.changPageLog(rec.get("shop_id").toString(), "funny", getPara("id"), "3");//记录更新日志
//				}
			}catch(Exception e){
				e.printStackTrace();
			}
	...
	public void save() {
		...
		boolean isSuccess = Db.tx(new IAtom(){public boolean run() throws SQLException {
			...
			if (rec.get("id") == null) {//新增
				...
				success = Db.save("bp_funny", rec.set("id", id).set("create_date",new Date()))
				...
				success = Db.update("bp_funny", rec);
				if(success){
		...
		if(isSuccess){
			...
			try{
				boolean success = DataSynUtil.addTask(rec.get("shop_id").toString(), "funny", actionRec.get("id").toString(), "1");
//				if(success){//这里不能基于succes判断是否执行下面代码，坑
					PageUtil.changPageLog(rec.get("shop_id").toString(), "funny", actionRec.get("id").toString(), actionRec.get("operate").toString());//记录更新日志
//				}
			}catch(Exception e){
				e.printStackTrace();
			}
	...
	private List<String> getOldImg(Map<String,List<File>> deleteRes,Object id){
		...
		try{
			Record oldRec = Db.findFirst("select img from bp_funny where id=?",new Object[]{id});
			if(null != oldRec && StringUtils.isNotBlank(oldRec.getStr("img"))){
				...
				routerDeleteRes.add("/storageroot/Data/"+FunnyTask.IMAGE_FOLDER+"/"+fileName);
	...
	private boolean synFile(Map<String,List<File>> deleteRes,List<String> routerDeleteRes,String taskDesc,Object id){
		...
		Record rec = Db.findFirst("select shop_id,ifnull(title,'') title from bp_funny where id=? ", new Object[]{id});
		String name = rec.getStr("title");
		if(StringUtils.isNotBlank(name)){
			name = "【"+name+"】";
		}
		Record taskInfo = new Record().set("task_desc", taskDesc+SynUtils.getAppName("funny")+name);
		Map<String,List<File>> res = FunnyTask.synRes(rec.get("shop_id"), sqls, taskInfo,routerDeleteRes,null);
		if(null != res){

...
function dictRadioDiaLog(dictId, dictName, checkedIds){
	$.ajax({
		type : "post",
		url : '${pageContext.request.contextPath}' + "/jf/dict/toUrl",
 ...
 * @param checkedIds
 */
function roleRadioDiaLog(roleId, roleName, checkedIds){
	$.ajax({
		type : "post",
		url : cxt+ "/system/role/",
		data : {"id" : checkedIds, "roleId" : roleId, "roleName" : roleName },
		dataType : "html",
		contentType: "application/x-www-form-urlencoded; charset=UTF-8",
		async: false,
		success:function(data){
...
function userRadioDiaLog(userId, userName, checkedIds){
	$.ajax({
		type : "post",
		url : '${pageContext.request.contextPath}' + "/jf/user/toUrl",

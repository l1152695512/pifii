...
<!-- 				<option selected="selected" value="">全部</option> -->
	...
	<div class="ft">
		<ul>
			...
			<li style="width:150px">操作时间</li>
			<li style="width:300px">操作</li>
			<li style="width:190px">状态</li>
...
// 	}
	function generPageChangeLogPager(isSearch){
		...
		$("#taskFlowList .holder").jPages({
			...
			realPagination:true,
	        serverParams:{
	        	url:"business/page/getPageChangeLog_old",
	        	data:data,
		...
		if(data.length > 0){
			...
			for(var i=0;i<data.length;i++){
				recHtml += '<ul>'+
								...
								'<li style="width: '+list_header.eq(1).width()+'px;">'+data[i].routerName+'</li>'+
								'<li style="width: '+list_header.eq(2).width()+'px;">'+data[i].operate_date+'</li>'+
								'<li style="width: '+list_header.eq(3).width()+'px;" title="'+data[i].operateDes+'">'+data[i].operateDes+'</li>'+
				...
				if(data[i].status == 0){
					...
					var tips = "";
					if(data[i].type=='introduce' && data[i].downloadSeconds != -1 && data[i].downloadSeconds < 4*60){//介绍应用中包含视频，需要更长时间
						tips = '正在更新资源';
					}else if(data[i].downloadSeconds < 1.5*60 && data[i].downloadSeconds != -1){
						tips = '正在更新资源';
					}else{
						tips = '已同步';
					}
					recHtml += '<span style="color:#5783E0;">'+tips+'</span>';
				}
				recHtml += '</li>'+
							'<li style="float: none;width: 0px;"></li>'+
						'</ul>';
